...
st.set_page_config(
    page_title="Knowledge Base Management - DocuBuddy", page_icon="📚", layout="wide"
)
...
def process_files(files, kb_name: str):
    ...
    try:
        ...
        rag_instance.process_documents(file_paths, kb_name=kb_name)
    ...
    with st.sidebar:
        ...
        if knowledge_bases:
            selected_kb = st.selectbox(
                ...
                index=(
                    knowledge_bases.index(st.session_state.get("current_kb"))
            ...
            if st.button("Process Documents", type="primary"):
                with st.spinner("Processing documents..."):
                    success, message = process_files(
                        uploaded_files, st.session_state.current_kb
                    )
                    if success:
                        st.success(message)
                    else:
        ...
        for kb_name in knowledge_bases:
            with st.expander(f"📚 {kb_name}", expanded=True):
                ...
                with col3:
                    st.metric(
                        "Last Modified",
                        datetime.fromtimestamp(kb_path.stat().st_mtime).strftime(
                            "%Y-%m-%d %H:%M"
                        ),
                    )
